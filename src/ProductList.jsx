...
function ProductList({ onHomeClick }) {
    ...
    // initialize addedToCart from store so buttons remain disabled for items already in cart
    useEffect(() => {
        const map = {};
        cartItems.forEach(i => { if (i && i.name) map[i.name] = true; });
        setAddedToCart(map);
    }, [cartItems]);
                    ...
                    <div>
                        <a href="#" onClick={(e) => handleCartClick(e)} style={styleA}>
                            <h1 className='cart'>
                                ...
                                { /* badge */ }
                                <span className="cart-count" style={{ position: 'relative', left: '-30px', top: '-50px', background: 'red', color: 'white', borderRadius: '50%', padding: '4px 8px', fontSize: '14px' }}>{cartItems.reduce((s, i) => s + (i.quantity || 0), 0)}</span>
                            </h1>
                        </a>
                    </div>
                </div>
                            ...
                            <div className="category-plants">
                                {category.plants.map((plant) => (
                                    <div className="product-card" key={`${category.category}-${plant.name}`}>
                                        ...
                                        >
                                            {addedToCart[plant.name] ? 'Añadido' : 'Añadir al Carrito'}
                                        </button>
                                    </div>
                                ))}
                            </div>
